...
### Eval gate for prompt/schema/model changes (`plan-generation-eval-set`)

A PR touching `resources/prompts/**`, `dto/llm/**`, or `llm-pricing.yml` must paste the **modo
candidato** eval table (`./mvnw -Peval test -Dtest=EvalRunnerTest -Dmodo=candidato`, requires
`OPENAI_API_KEY`) in the PR description — not the modo auditoria table, which only audits past
production data and never re-runs the candidate prompt.

The juiz-LLM column is marked **"NÃO CALIBRADO"** in this version (no human-scored calibration set
yet — see `plan-generation-eval-set` proposal Open Questions). Treat it as a complementary signal,
never isolated evidence of approval; the grader determinístico (`PlanQualityChecker` +
`SkeletonComplianceChecker`, via `PlannerShadowService`) is the primary evidence for this gate.
