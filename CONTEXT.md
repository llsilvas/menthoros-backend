...
Consolidação retrospectiva de uma semana de treino do atleta, **1:1 com o `PlanoSemanal`** — não um conceito de "semana" à parte (reusa `semana_inicio/fim`, `tsb_inicio/fim`, volumes do plano). Gerada e congelada no encerramento da semana (`CONCLUIDO` via `EncerramentoSemanaService`, manual ou fallback automático), guardando o **sinal estruturado do que foi proposto ao coach**: `recommendationType` (RECOVERY/MAINTAIN/PROGRESS, derivado da aderência + `tsb_fim`), `adherenceSummary.status` (por contagem, alinhado ao `aderenciaPercentual` do roster) e `dadosSuficientes`. `weekOverWeekDelta` é computado (diff contra o `PlanoSemanal` anterior), não persistido. Fatiada em 3: `add-weekly-review-consolidation` (este sinal determinístico congelado), `add-weekly-review-llm-focus` (narrativa `nextWeekFocus` por LLM + `focusOutcome`) e `add-weekly-review-coach-card` (card read-only no drilldown do atleta).
_Avoid_: janela de revisão com `semanaInicio`/`semanaFim` próprios (usar os do `PlanoSemanal`); `confidence` para suficiência de dado (colide com `ConfidenceTier` — o campo é `dadosSuficientes`); aderência por TSS na revisão (o número canônico é a contagem do roster)
