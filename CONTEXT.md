...
**Revisão Semanal** (`RevisaoSemanal`, capability `weekly-athlete-review`, decisão 2026-07-24, ver ADR-0006):
Consolidação retrospectiva de uma semana de treino do atleta, **1:1 com o `PlanoSemanal`** — não um conceito de "semana" à parte (reusa `semana_inicio/fim`, `tsb_inicio/fim`, volumes do plano). Gerada e congelada no encerramento da semana (`CONCLUIDO` via `EncerramentoSemanaService`, manual ou fallback automático), guardando o **sinal estruturado do que foi proposto ao coach**: `recommendationType` (RECOVERY/MAINTAIN/PROGRESS, derivado da aderência + `tsb_fim`), `adherenceSummary.status` (por contagem, alinhado ao `aderenciaPercentual` do roster) e `sufficientData`. `weekOverWeekDelta` é computado (diff contra o `PlanoSemanal` anterior), não persistido. A revisão é **estritamente a proposta**: nada do que o coach faz depois é escrito nela (ver Desfecho da Revisão Consumida). Fatiada em 3: `add-weekly-review-consolidation` (este sinal determinístico congelado), `add-weekly-review-llm-focus` (narrativa por LLM + loop de aprendizado) e `add-weekly-review-coach-card` (card read-only no drilldown do atleta).
_Avoid_: janela de revisão com `semanaInicio`/`semanaFim` próprios (usar os do `PlanoSemanal`); `confidence` para suficiência de dado (colide com `ConfidenceTier` — o campo é `sufficientData`); aderência por TSS na revisão (o número canônico é a contagem do roster); `dadosSuficientes`/`percentualRealizacao` (nomes da F1, renomeados para `sufficientData`/`completionRate` em 2026-07-25)

**Foco da Próxima Semana** (`nextWeekFocus`, decisão 2026-07-25):
A narrativa de uma frase que a Revisão Semanal propõe ao coach como foco da semana seguinte. Existe **sempre** que há revisão — com a flag do LLM ligada é redigida por IA restrita ao `recommendationType`; desligada (ou quando o LLM falha, ou quando a narrativa é reprovada pelo checker de consistência) é um template determinístico derivado do mesmo `recommendationType`. Nunca é exposta ao atleta.

**Origem do Foco** (`focusSource`: `LLM`|`TEMPLATE`, decisão 2026-07-25):
Registra *como* o Foco da Próxima Semana daquela revisão foi produzido. Mesma razão da Origem da Aprovação: uma vez persistido, um foco escrito por LLM e um template são indistinguíveis, e sem a distinção não há como medir se a narrativa por IA muda o comportamento do coach — que é a contrapartida de valor do gate de custo A1. Sem esse campo, a decisão de ligar a flag em definitivo teria custo de um lado e nenhuma medida de benefício do outro.

**Desfecho da Revisão Consumida** (`consumedReviewOutcome`, campo do **`PlanoSemanal`**, decisão 2026-07-25):
O que um plano fez com a Revisão Semanal que ele consumiu como insumo: `PENDING` (consumida, plano ainda não decidido), `NO_ADJUSTMENT` (aprovado pelo coach sem treino editado/adicionado), `ADJUSTED` (aprovado pelo coach com treino editado/adicionado), `PLAN_REJECTED` (coach rejeitou o plano), `NOT_CONSUMED` (a revisão não entrou no insumo — flag desligada, ausente ou fora da janela de validade), `NO_COACH_IN_LOOP` (plano auto-aprovado por `AUTO_CONFIANCA_ALTA` — não há coach para julgar o foco).
Vive no **plano**, não na revisão, por duas razões: uma revisão é 1:N com os planos que a consomem (rejeitar e regerar faz dois planos consumirem a mesma revisão — no campo único da revisão o segundo desfecho sobrescreveria o primeiro, apagando justamente a rejeição que interessa ao moat); e manter a revisão sem escrita pós-congelamento preserva o espírito do ADR-0006.
É um **proxy de segunda ordem, não um julgamento do coach sobre o foco**: mede "algum treino deste plano foi mexido", que capta tanto discordância do foco quanto ajuste por logística (atleta viajou, mudou de horário). Serve como taxa agregada de adoção; não deve ser lido como decisão individual do coach. Uma superfície de captura explícita pode substituí-lo depois sem mudar o contrato do campo.
_Avoid_: `focusOutcome` (nome inicial, rejeitado por prometer um sinal de primeira ordem que a heurística não entrega); ler `ADJUSTED` como "o coach discordou do foco"
