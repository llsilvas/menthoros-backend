...
Declaração do atleta de qual plataforma ele vai usar para enviar/receber treinos: `INTERVALS_ICU` (com Garmin como dispositivo prioritário na orientação de conexão) ou `MANUAL` (sem integração, upload de .fit). **Não é "marca do relógio"** — a marca (Garmin/Coros/Polar/...) não determina qual canal o Menthoros usa (o push de treino planejado já é agnóstico de marca via `WorkoutChannel`/`IntervalsIcuAdapter`); o que varia de verdade é a plataforma escolhida. Strava não é oferecido como opção para atletas novos (ver ADR-0003) — em descontinuação, mas ainda ativo para quem já está conectado.
_Avoid_: "relógio do atleta", "dispositivo" (o dispositivo/marca não é o dado relevante — é o canal)
